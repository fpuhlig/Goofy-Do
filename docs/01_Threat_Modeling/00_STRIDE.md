...
## 1 Komponenten
### 1.1 Frontend
| ID  | STRIDE | Beschreibung | Beispielangriff | Vorraussetzungen | Gegenmaßnahmen | Status        |
|-----| --- | --- | --- | --- | --- |---------------|
...
| F03 | Repuditation | Fehlende request-bezogene Logs | Fehlende Logs verhindern Zuordnung von UI-Aktionen zu Usern | Kein Request-Logging | Zentralisiertes Logging | Akzeptiert    |
...
| B03 | Repuditation | Änderungen nicht nachvollziehbar                                                 | Unklar, wer Datensatz angelegt/geändert hat | Kein Audit-Feld, kein Änderungs-Log                                                    | Audit-Tabellen, Änderungs-Metadaten |               |
...
| K03 | Repuditation | Änderungen an Identitäten nicht nachvollziehbar | Unklar, wer wann welche Rolle verändert oder Client registriert hat | Kein Audit-Log an Keycloak | Aktivierte Admin-/Auth-Logs, zentrale Log-Aggregation | Implementiert |
...
| DB03 | Repuditation | Änderungen an Daten nicht nachvollziehbar | Unklar, wer sensible Datensätze bearbeitet oder gelöscht hat | Kein DB-Audit, keine Änderungs-Historie                                                  | DB-Audit-Logging, Change-Tables                                                       | Implementiert |
...
| CF03 | Repuditation | User-Aktionen nicht nachweisbar | Nutzer bestreitet Änderung, Logs fehlen | Keine korrelierten Logs | Request-/Audit-Logging mit User-ID, Zeit, IP, Signatur wichtiger Transaktionen | Implementiert |
...
| EG03 | Repuditation | Commit-Herkunft bestreitbar | Developer streitet commit ab, Identität unklar | Keine Signaturen | GPG/SSH-Commit-Signaturen, Logs auf GitHub | Implementiert |
...
| EG06 | Elevation of privilege | CI/CD über Code-Push missbraucht | Änderung von Pipeline-Configs im Repo führt zu mehr Rechten | CI nutzt Repo-Konfiguration ungeprüft | Separate Infrastruktur Repos, Reviews für CI-Config, Policys für ändern von Pipelines | Akzeptiert    |
